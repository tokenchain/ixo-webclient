...
export class Verification extends React.Component<ParentProps, State> {
	...
	componentDidMount() {
		this.checkState();
		if (this.state.hasKeySafe === false || this.state.isDidLedgered === false || !!this.state.didDoc === false) {
	...
	checkState() {
		...
		} else if (this.props.ixo && this.state.didDoc) {
			console.log('test');
	...
	getLedgeredText = () => {
		return (
			<CheckItem>
				{this.getIcon(this.state.isDidLedgered)} <strong><a onClick={this.ledgerDid} target="_blank">Register your identity</a> on the ixo blockchain</strong>
